...
extraInfoHTML(){
  return `<p> <a href="${this.instructionsLink}" target= "_blank">Instructions</a>
...
async function loadProductsFetch(){
  ...
  loadedProducts = JSON.parse(localStorage.getItem('loadedProducts'))|| await response.json(); 
  localStorage.setItem('loadedProducts',JSON.stringify( loadedProducts))
    }catch{(error)=>{
...
await loadProductsFetch().then(()=>{
  products = loadedProducts.map((productDetails)=>{
